...
# 1. START PAGE
# =============================================================================
with st.expander("**1. Start page – Sessions, questionnaire & population**", expanded=(section_param == SECTION_START)):
...
# =============================================================================
with st.expander("**2. Answer options – Likert scales & response format**", expanded=(section_param == SECTION_OPTIONS)):
    ...
    with st.container(border=True):
        ...
        with _col2:
            ...
            st.caption("How options are labeled: integer (1,2,3…), char_low (a,b,c…), or char_up (A,B,C…).")
        with _col3:
        ...
        with _co1:
            ...
            st.caption("Only first and last labels shown (e.g. “1 Strongly Disagree to 5 Strongly Agree”). Requires exactly 2 answer texts.")
        with _co2:
        ...
        st.markdown("""
        - **None** — Model returns free text; no structured output. You must parse the response yourself. No extra configuration on this page.
        - **JSON Single Answer** — Model returns a single selected answer in JSON (e.g. one chosen option per question). Parsing is automatic; use when you want one answer per item.
        - **JSON All Options (Probabilities)** — Model returns a probability (or score) for each option. Use when you want a distribution over choices rather than a single pick. Parsing expects this structure.
        - **JSON with Reasoning** — Model returns reasoning text plus the chosen answer in JSON. Use when you need both explanation and a structured answer. Parsing extracts both.
        - **Choice** — Model must pick exactly one value from a fixed list. When selected, **Allowed Choices** appears: one choice per line, either indices only (e.g. 1, 2, 3) or with labels (e.g. 1: Strongly Disagree). **Output Indices Only** controls whether the model returns indices or full labels. Parsing uses the same list.
        """)
    st.subheader("Next step")
...
# =============================================================================
with st.expander("**3. Prompt configuration – System prompt & instructions**", expanded=(section_param == SECTION_PROMPTS)):
    ...
    with st.container(border=True):
        ...
        st.caption("Template for each question. Use **Question Content** placeholder (-Q) so the actual item (e.g. “Coffee”) is inserted.")
    ...
    st.subheader("Next step")
    st.page_link("pages/02_Prompt_Configuration.py", label="→ Go to Prompt configuration")

# =============================================================================
# 4. INFERENCE SETTINGS
# =============================================================================
with st.expander("**4. Inference settings – API client & model**", expanded=(section_param == SECTION_INFERENCE)):
...
# =============================================================================
with st.expander("**5. Final overview – Run survey & save results**", expanded=(section_param == SECTION_OVERVIEW)):
    ...
    with st.container(border=True):
        ...
        st.markdown("**Details by option:**")
        st.markdown("""
        - **Single item** — One question is sent per API call; the model sees only that question (and system/instruction context). Preview on the page shows the first few items so you can check how each is prompted.
        - **Battery** — All questions are sent in one API call; the model sees the full list and responds once. Preview shows one combined prompt.
        - **Sequential** — All questions are sent in one conversation thread with history; the model sees prior Q&A. Preview shows one combined flow. Parsing respects the conversation structure.
        """)
...
st.markdown("**Start** (upload data, confirm) → **Answer options** (scale + response method, confirm) → **Prompt configuration** (prompts + placeholders, confirm) → **Inference settings** (API + model, generate config) → **Final overview** (run & save).")
